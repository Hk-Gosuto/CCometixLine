...
const version =
  process.env.GITHUB_REF?.replace("refs/tags/v", "") || process.argv[2];
if (!version) {
...
platforms.forEach((platform) => {
  ...
  console.log(`✓ Prepared @gosuto/ccline-${platform} v${version}`);
});
...
if (mainPackageJson.optionalDependencies) {
  Object.keys(mainPackageJson.optionalDependencies).forEach((dep) => {
    if (dep.startsWith("@gosuto/ccline-")) {
...
console.log(`✓ Prepared @gosuto/ccline v${version}`);
